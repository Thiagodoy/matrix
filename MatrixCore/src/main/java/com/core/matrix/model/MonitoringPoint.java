...
@NamedNativeQuery(name = "MonitoringPoint.getStatusByContract",
        ...
        + "            '' AS status\n"
        + "    FROM\n"
        + "        matrix.mtx_contrato_informacao_complementar f\n"
        + "    WHERE\n"
        + "        f.wbc_codigo_contrato IN (SELECT DISTINCT\n"
        + "                x.wbc_codigo_contrato_rateio\n"
        + "            FROM\n"
        + "                matrix.mtx_arquivo_de_medicao a\n"
        + "            INNER JOIN matrix.mtx_contrato_informacao_complementar x ON x.wbc_contrato = a.wbc_contrato\n"
        + "                AND a.ano = :year\n"
        + "                AND a.mes = :month\n"
        ...
        + "    GROUP BY a.id_arquivo_de_medicao , a.wbc_ponto_de_medicao , a.empresa_nome , a.empresa_apelido , c.NAME_) xx\n"
